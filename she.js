...
;((generator) => {
	...
	const setup = (exports, curveType, range, tryNum) => {
		...
		const callMul = (func, x, y) => {
			if (
				!exports.CipherTextG1.prototype.isPrototypeOf(x) ||
				!exports.CipherTextG2.prototype.isPrototypeOf(y)
			)
				throw 'exports.mul:bad type'
		...
		mod.sheSecretKeyDeserialize = _wrapDeserialize(
			mod._sheSecretKeyDeserialize,
		)
		mod.shePublicKeySerialize = _wrapSerialize(mod._shePublicKeySerialize)
		mod.shePublicKeyDeserialize = _wrapDeserialize(
			mod._shePublicKeyDeserialize,
		)
		mod.sheCipherTextG1Serialize = _wrapSerialize(
			mod._sheCipherTextG1Serialize,
		)
		mod.sheCipherTextG1Deserialize = _wrapDeserialize(
			mod._sheCipherTextG1Deserialize,
		)
		mod.sheDecG1 = wrap_dec(mod._sheDecG1)
		mod.sheDecG1ViaGT = wrap_dec(mod._sheDecG1ViaGT)
		mod.sheCipherTextG2Serialize = _wrapSerialize(
			mod._sheCipherTextG2Serialize,
		)
		mod.sheCipherTextG2Deserialize = _wrapDeserialize(
			mod._sheCipherTextG2Deserialize,
		)
		mod.sheDecG2 = wrap_dec(mod._sheDecG2)
		mod.sheDecG2ViaGT = wrap_dec(mod._sheDecG2ViaGT)
		mod.sheCipherTextGTSerialize = _wrapSerialize(
			mod._sheCipherTextGTSerialize,
		)
		mod.sheCipherTextGTDeserialize = _wrapDeserialize(
			mod._sheCipherTextGTDeserialize,
		)
		...
		class Common {
			...
			_save(pos) {
				this.a_.set(
					mod.HEAP32.subarray(pos / 4, pos / 4 + this.a_.length),
				)
			}
			// save and free
			_saveAndFree(pos) {
		...
		exports.SecretKey = class extends Common {
			...
			decWithZkpDec(c, pub) {
				...
				const pubPos = pub._allocAndCopy()
				const r = mod._sheDecWithZkpDecG1(
					mPos,
					zkpPos,
					secPos,
					cPos,
      ...
      */
			...
			encG1(m) {
				return callPPKEnc(
					mod._shePrecomputedPublicKeyEncG1,
					exports.CipherTextG1,
					this.p,
					m,
				)
			}
			encG2(m) {
				return callPPKEnc(
					mod._shePrecomputedPublicKeyEncG2,
					exports.CipherTextG2,
					this.p,
					m,
				)
			}
			encGT(m) {
				return callPPKEnc(
					mod._shePrecomputedPublicKeyEncGT,
					exports.CipherTextGT,
					this.p,
					m,
				)
			}
			// return [Enc(m), Zkp]
			encWithZkpBinG1(m) {
				return callPPKEncWithZkpBin(
					mod._shePrecomputedPublicKeyEncWithZkpBinG1,
					exports.CipherTextG1,
					this.p,
					m,
				)
			...
			encWithZkpBinG2(m) {
				return callPPKEncWithZkpBin(
					mod._shePrecomputedPublicKeyEncWithZkpBinG2,
					exports.CipherTextG2,
					this.p,
					m,
				)
		...
		exports.PublicKey = class extends Common {
			...
			// return [Enc(m), Zkp]
			encWithZkpBinG1(m) {
				return callEncWithZkpBin(
					mod._sheEncWithZkpBinG1,
					exports.CipherTextG1,
					this,
					m,
				)
		...
		console.log(
			`initializing sheSetRangeForDLP(range=${range}, tryNum=${tryNum})`,
		)
  ...
  */
	exports.init = (
		...
		range = 1024,
		tryNum = defaultTryNum,
	) => {
		if (curveType > 8) {
			console.log(
				'WARNING : init(range, tryNum) is deprecated. use init(curveType, range, tryNum)',
			)
			tryNum = range
			range = curveType
			curveType = exports.BN254
